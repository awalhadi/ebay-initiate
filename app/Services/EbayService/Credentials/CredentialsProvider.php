<?php
namespace App\Services\EbayService\Credentials;
...
class CredentialsProvider
{
    ...
    /**
     * Provider that fetches credentials from a database and falls back to other sources if not found.
     *
     * @return callable
     */
    public static function database()
    {
        return function () {
          $userId = Auth::id(); // Get the authenticated user's ID
            ...
            // Retrieve credentials from the database if available
            // $credentialsFromDB = EbayCredentials::first(); // Fetch credentials from your database
            $credentialsFromDB = (object)[
              'app' => '',
              'cert_id' => '',
              'dev_id' => '',
            ]; // Fetch credentials from your database

            if ($credentialsFromDB) {
                return new Credentials(
                    $credentialsFromDB->app_id,
                    $credentialsFromDB->cert_id,
                    $credentialsFromDB->dev_id
                );
            }

            // If credentials are not found in the database, fallback to other providers
            return self::chain(
                self::env(), // Attempt to get credentials from environment variables
                self::ini()  // Attempt to get credentials from the INI file
                // Add more providers if needed for fallback
            )();
        };
    }
