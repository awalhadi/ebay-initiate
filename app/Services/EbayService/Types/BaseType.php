...
use App\Services\EbayService\JmesPath\Env;
use App\Services\EbayService\Exceptions\InvalidPropertyTypeException;

/**
 * Base class for handling properties and XML conversion.
 */
class BaseType
{
  /**
    ...
    public function toArray()
    {
      ...
      foreach (static::$properties[static::class] as $name => $info) {
          ...
          if ($info['repeatable']) {
              if (count($value)) {
                  ...
                  foreach ($value as $property) {
                      $array[$name][] = $this->propertyToArrayValue($property);
                  }
              }
          } else {
              $array[$name] = $this->propertyToArrayValue($value);
          }
      }
    ...
    {
      if (!array_key_exists($name, static::$properties[$class])) {
